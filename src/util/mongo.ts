...
export function useMongo(): mongoose.Connection {
  const db = mongoose.createConnection(
    "mongodb://root:example@localhost:27017/node-system?authSource=admin",
  );
...
export function useMessageFileRepository(db: mongoose.Connection) {
  return {
    async create(file: NodeJS.ReadableStream): Promise<string> {
      if (db.db == null) {
      ...
      return new Promise((resolve, reject) => {
        ...
        uploadStream.on("finish", async () => {
          ...
          console.log({ fileIDStr, length }, "File uploaded");
          resolve(fileIDStr);
        });
